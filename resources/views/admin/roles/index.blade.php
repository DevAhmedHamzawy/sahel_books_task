        ...
        <div class="card mg-b-20">
            <div class="card-header pb-0">
                <div class="d-flex justify-content-between">
                    ...
                    <div class="btn btn-primary"><a class="text-white" href="{{ route('roles.create') }}">{{ trans('roles.add_new_role') }}</a></div>
                </div>
            </div>
            <div class="card-body">
                            ...
                            @foreach ($roles as $role)
                                <tr>
                                    ...
                                    @canany(['edit_role', 'delete_role'])
                                        <td class="row pl-3">
                                            @can('edit_role')
                                                <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-warning">{{ trans('dashboard.edit') }}</a>
                                            @endcan
                                            &nbsp;&nbsp;
                                            @can('delete_role')
                                                <form class="pl-3" action="{{ route('roles.destroy', $role->id) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
